...
export const ProgressBar = ({ numberOfStepsCompleted, numberOfSteps }: ProgressBarProps) => {
  ...
  while(i < numberOfPoints){
    progressLength.push(
      <View className="bg-blue-700 h-2 mt-1 flex w-1"></View>
    )
    i++;
  }
    ...
    <View>
      ...
      <View className="flex-row mt-2">
        <View className="min-w-full"> 
          <View ref={targetRef} className="rounded-full bg-gray-400 py-1 px-4"></View>
        </View>
        <View className="rounded-full -ml-6 h-8 w-8 bg-gray-400 flex items-center justify-center -mt-3">
          {completionTick}
        </View>
      </View>
      <View className="flex-row min-w-full -mt-6">
        <View className="mt-1 rounded-l bg-blue-700 h-2 flex w-2"></View>
        {progressLength}
        <View className="mt-1 rounded-r bg-blue-700 h-2 flex w-2"></View>
      </View>
    </View>
  );
};
