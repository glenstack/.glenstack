...
const bookType = new GraphQLFaunaObjectType({
  ...
  metaSchema:  {title: { fieldId: "294845251673129473", type:"string"}, authors: { fieldId: "294845329476420097", relationshipRef: q.Ref(q.Collection("relationships"), "296152190589862405"), type:"relation",  relation: "A"}}
})
...
const authorType = new GraphQLFaunaObjectType({
  ...
  metaSchema:  {name: { fieldId: "294845354656924161", type:"string"}, books: { fieldId: "294845383336526337",relationshipRef: q.Ref(q.Collection("relationships"), "296152190589862405"), type:"relation",  relation: "B"}}
})
    ...
    resolve: async (obj: any, args, context: any, info: any) => {
      ...
      for (let [key, value] of Object.entries(args.input)) {
        if (bookType.metaSchema[key].type === "relation") {
          ...
          relationQueries = q.Create(q.Collection("relations"),{data: {
            relationshipRef: bookType.metaSchema[key].relationshipRef,
            [bookType.metaSchema[key].relation]:  q.Var("docRef"),
            [bookType.metaSchema[key].relation === "A" ? "B" : "A"] : q.Ref(q.Collection("294845159814726145"),value.connect[0])
          }})
        }
        else {
      ...
      let res = await client.query(q.Let({docRef: q.Select(['ref'],q.Create(q.Collection(bookType.collectionName),{data}))},{finished: relationQueries}))
